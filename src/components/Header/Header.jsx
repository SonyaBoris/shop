      ...
      <div className={s.info}>
        ...
        <form className={s.form}>
          <div className={s.icon}>
            <svg className='icon' fill="inherit" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 13 13" id="search">
          ...
          {searchValue && <div className={s.box}>
            {isLoading ? 'Loading' : !data.length ? 'No results' : (
              data.map(({ title, images, id }) => {
                return (
                  <Link onClick={() => setSearchValue("")} className={s.item} to={`/products/${id}`} key={id}>
        ...
        <div className={s.account}>
          <Link to={ROUTES.HOME} className={s.favourites}>
            <svg fill="inherit" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" id="heart">
              <path d="M3 12C0.75 9 1.5 4.5 5.25 3C9 1.5 11.25 4.5 12 6C12.75 4.5 15.75 1.5 19.5 3C23.25 4.5 23.25 9 21 12C18.75 15 12 21 12 21C12 21 5.25 15 3 12Z" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"></path>
            </svg>
          </Link>

          <Link to={ROUTES.CART} className={s.cart}>
            <svg fill="inherit" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" id="bag">
